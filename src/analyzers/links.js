...
async function analyzeLinks($, baseUrl, options = {}) {
  ...
  check(results, {
    ...
    failMsg:
      brokenCount === 1
        ? `1 broken link mila — fix karo`
        : `${brokenCount} broken links mile — fix karo!`,
    passMsg:
      totalChecked > 0
        ? `Koi broken link nahi (${totalChecked} links checked) ✓`
        : "Koi checkable link nahi mila",
  ...
  check(results, {
    ...
    failMsg: `${redirectCount} links redirect kar rahe hain — URLs update karo`,
    passMsg:
      redirectCount === 0
        ? "Koi unnecessary redirect nahi ✓"
        : `${redirectCount} redirects hain (acceptable) ✓`,
  ...
  check(results, {
    ...
    failMsg:
      results.data.internal.length === 0
        ? "Koi internal link nahi — SEO ke liye internal linking zaroori hai"
        : `Sirf ${results.data.internal.length} internal links — aur add karo`,
    passMsg: `${results.data.internal.length} internal links hain ✓`,
  ...
  check(results, {
    ...
    failMsg: "Koi external link nahi — trusted sources link karo",
    passMsg: `${results.data.external.length} external links hain ✓`,
    isWarning: true,
  });
  ...
  if (results.data.nofollow.length > 0) {
    check(results, {
      ...
      pass: true,
      failMsg: "",
      passMsg: `${results.data.nofollow.length} nofollow links hain (OK) ✓`,
    });
  }
