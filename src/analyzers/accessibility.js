...
function analyzeAccessibility($) {
  ...
  check(results, {
    ...
    weight: 8,
    pass: !!htmlLang,
    failMsg: '<html lang="..."> missing — screen readers ko language nahi pata chalega',
    passMsg: `Language set hai: "${htmlLang}" ✓`,
  });
  ...
  if (totalImgs > 0) {
    check(results, {
      ...
      failMsg: `${missingAlt.length}/${totalImgs} images mein alt attribute nahi`,
      passMsg: `Sab ${totalImgs} images mein alt text hai ✓`,
  ...
  check(results, {
    ...
    weight: 10,
    pass: h1Count >= 1,
    failMsg: "Page mein koi H1 nahi — main heading missing hai",
    passMsg: `H1 heading present hai ✓`,
  });
  ...
  check(results, {
    ...
    weight: 6,
    pass: h1Count <= 1,
    failMsg: `${h1Count} H1 tags hain — sirf 1 hona chahiye`,
    passMsg: "Ek hi H1 hai ✓",
    isWarning: true,
  });

  // Heading skip check (H1 → H3 skip karna galat hai)
  ...
  check(results, {
    ...
    weight: 7,
    pass: !headingSkip,
    failMsg: "Heading levels skip ho rahe hain (e.g. H1 → H3) — H2 missing",
    passMsg: "Heading hierarchy sahi order mein hai ✓",
    isWarning: true,
  });
  ...
  if (totalInputs > 0) {
    check(results, {
      ...
      failMsg: `${unlabeledInputs}/${totalInputs} form inputs unlabeled — screen readers ke liye problem`,
      passMsg: `Sab ${totalInputs} form inputs labeled hain ✓`,
  ...
  if (totalButtons > 0) {
    check(results, {
      ...
      weight: 8,
      pass: emptyButtons === 0,
      failMsg: `${emptyButtons} buttons mein text/aria-label nahi — screen readers ke liye invisible`,
      passMsg: `Sab ${totalButtons} buttons accessible hain ✓`,
  ...
  results.data.links = { total: totalLinks, empty: emptyLinks, generic: genericLinks };

  if (totalLinks > 0) {
    check(results, {
      label: "Links have descriptive text",
      weight: 7,
      pass: emptyLinks === 0 && genericLinks <= 2,
      failMsg: [
        emptyLinks > 0 ? `${emptyLinks} empty links` : "",
        genericLinks > 2 ? `${genericLinks} generic links ("click here" etc)` : "",
      ]
        .filter(Boolean)
        .join(", "),
      passMsg: `Links descriptive hain ✓`,
      isWarning: genericLinks > 0 && genericLinks <= 2,
    });
  }
  ...
  check(results, {
    ...
    weight: 5,
    pass: skipLink,
    failMsg: 'Skip to main content link missing — keyboard users ke liye helpful hota',
    passMsg: "Skip navigation link present hai ✓",
    isWarning: true,
  });
  ...
  check(results, {
    ...
    failMsg:
      landmarkCount === 0
        ? "Koi ARIA landmark nahi — <main>, <nav>, <header> use karo"
        : `Sirf ${landmarkCount}/4 landmarks — <main>, <nav>, <header>, <footer> add karo`,
    passMsg: `${landmarkCount}/4 ARIA landmarks hain ✓`,
  ...
  check(results, {
    ...
    weight: 5,
    pass: badTabindex === 0,
    failMsg: `${badTabindex} elements mein tabindex > 0 — tab order disturb hoga`,
    passMsg: "tabindex sahi use ho raha hai ✓",
    isWarning: true,
  });
  ...
  check(results, {
    ...
    weight: 8,
    pass: !zoomDisabled,
    failMsg: "Viewport mein zoom disable hai — visually impaired users ke liye problem!",
    passMsg: "User zoom enabled hai ✓",
  });
