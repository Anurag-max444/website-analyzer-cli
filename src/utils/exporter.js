...
function buildHTMLReport(url, results, summary) {
  const { seo, performance, accessibility, links, techStack } = results;
...
function resolveOutputPath(outputPath, ext) {
  if (!outputPath) {
    ...
    return path.resolve(`website-report-${timestamp}.${ext}`);
  }
  if (!outputPath.endsWith(`.${ext}`)) return outputPath + `.${ext}`;
