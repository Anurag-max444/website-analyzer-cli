...
const { analyze } = require("@website-analyzer/core");
const { setLang } = require("@website-analyzer/core/src/utils/lang");
...
program
  ...
  .option("--json", "Output results as JSON in terminal")
  .option(
    "--only <modules>",
    "Run only specific modules (comma-separated: seo,performance,tech,accessibility,links)",
    parseModules
  )
  .option("--export <path>", "Save report to file (e.g. --export ./report)")
  .option(
    "--format <format>",
    "Export format: html | json | both (default: html)",
    "html"
  )
  .option("--no-links", "Skip link checking (faster results)")
  .option("--silent", "Hide spinners and progress output")
  .option("--lang <lang>", "Language: en (default) | hi (Hinglish)", "en")
  .addHelpText(
    "after",
    `
...
${chalk.bold.yellow("Export Formats:")}
  ...
  ${chalk.green("both")}            Export both HTML and JSON

${chalk.bold.yellow("Language:")}
  ${chalk.green("--lang en")}       English output (default)
  ${chalk.green("--lang hi")}       Hinglish output
    `
  );
...
program.action(async (url, options) => {
  // Set language FIRST — before anything else runs
  ...
  try {
    ...
    // Helpful hints
    if (err.message.includes("not found") || err.message.includes("nahi mila") || err.message.includes("ENOTFOUND")) {
      console.error(chalk.gray("  → Check the URL and your internet connection"));
    } else if (err.message.includes("Invalid URL")) {
      console.error(chalk.gray("  → Example: website-analyzer https://google.com"));
    } else if (err.message.includes("Timeout")) {
      console.error(chalk.gray("  → Site is too slow or not responding"));
    }

    console.error(chalk.gray("\n  For help, run: website-analyzer --help\n"));
    process.exit(1);
  }
});
...
function parseModules(val) {
  const valid = ["seo", "performance", "tech", "accessibility", "links", "security", "sitemap", "robots", "schema"];
