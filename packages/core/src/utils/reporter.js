/**
 * Reporter — Prints colorful terminal output
...
function printReport(url, { seo, performance, techStack, accessibility, links, security, sitemap, robots, schema }, responseTime) {
  printHeader(url, responseTime);
  printOverallScore({ seo, performance, accessibility, links, security, sitemap, robots, schema });
  ...
  printSection("Sitemap", sitemap);
  printSection("Robots", robots);
  printSection("Schema", schema);
  printFooter();
}
...
function printHeader(url, responseTime) {
  ...
  console.log(chalk.cyan("│") +
    chalk.bold.white(`  ${t("reporter.title")}`) +
    " ".repeat(60 - 30) +
    chalk.cyan("│"));
...
function printOverallScore({ seo, performance, accessibility, links, security, sitemap, robots, schema }) {
  console.log(chalk.bold.white(`${t("reporter.overallScores")}\n`));
  ...
  const categories = [
    ...
    { name: "Sitemap", data: sitemap },
    { name: "Robots", data: robots },
  ...
  table.push([
    chalk.bold.white(t("reporter.catOverall")),
...
function printTechStack(techStack) {
  if (!techStack || techStack.count === 0) {
    console.log(chalk.bold.white(`${t("reporter.techStack")}\n`));
    console.log(chalk.gray(`  ${t("reporter.noTechFound")}\n`));
    return;
  }
...
function printSection(name, data) {
  ...
  const sectionTitles = {
    SEO: t("reporter.seoAnalysis"),
    Performance: t("reporter.perfAnalysis"),
    Accessibility: t("reporter.a11yAnalysis"),
  };

  console.log(
    chalk.bold.white(`${sectionTitles[name] || `📋 ${name.toUpperCase()} ANALYSIS`}\n`)
  );

  // Issues
  if (data.issues.length > 0) {
    console.log(chalk.red(`  ${t("reporter.issues")}`));
  ...
  if (data.warnings.length > 0) {
    console.log(chalk.yellow(`  ${t("reporter.warnings")}`));
  ...
  if (data.passed.length > 0) {
    console.log(chalk.green(`  ${t("reporter.passed")}`));
...
function printLinksSection(links) {
  ...
  console.log(chalk.bold.white(`${t("reporter.linksAnalysis")}\n`));

  // Summary table
  const table = new Table({
    head: [chalk.bold.cyan(t("reporter.colLinkType")), chalk.bold.cyan(t("reporter.colCount"))],
    colWidths: [25, 15],
    style: { border: ["cyan"], head: [] },
  });

  table.push([t("reporter.totalLinks"), chalk.white(links.data.total)]);
  table.push([t("reporter.internalLinks"), chalk.white(links.data.internal.length)]);
  table.push([t("reporter.externalLinks"), chalk.white(links.data.external.length)]);
  table.push([
    t("reporter.brokenLinksCount"),
    links.data.broken.length > 0
      ? chalk.red(links.data.broken.length + " ❌")
      : chalk.green("0 ✓"),
  ]);
  table.push([t("reporter.redirectCount"), links.data.redirects.length > 0 ? chalk.yellow(links.data.redirects.length) : chalk.green("0 ✓")]);
  table.push([t("reporter.nofollowCount"), chalk.white(links.data.nofollow.length)]);
  if (links.data.mailto.length > 0)
    table.push([t("reporter.mailtoLinks"), chalk.white(links.data.mailto.length)]);
  if (links.data.tel.length > 0)
    table.push([t("reporter.telLinks"), chalk.white(links.data.tel.length)]);
  if (links.data.skipped > 0)
    table.push([t("reporter.skipped"), chalk.gray(links.data.skipped)]);

  console.log(table.toString());

  // Broken links detail
  if (links.data.broken.length > 0) {
    console.log(chalk.red(`\n  ${t("reporter.brokenLinks")}`));
    ...
    if (links.data.broken.length > 10) {
      console.log(chalk.gray(`    ${t("reporter.moreLinks")(links.data.broken.length - 10)}`));
    }
  }

  // Redirects detail
  if (links.data.redirects.length > 0) {
    console.log(chalk.yellow(`\n  ${t("reporter.redirects")}`));
